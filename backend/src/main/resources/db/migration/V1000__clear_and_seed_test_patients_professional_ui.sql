-- Re-create test patient records after manual resets / UI rebuilds.
-- This is intentionally a NEW migration version so Flyway will run it again.
-- It reseeds the same patient set used for the registration completeness UI.
...
-- Some environments may not have every insurance/consent-related table.
DO $$
BEGIN
...
-- Reset patient_id sequence (optional for clean IDs)
ALTER SEQUENCE IF EXISTS patients_patient_id_seq RESTART WITH 1;

-- Seed new test patients
